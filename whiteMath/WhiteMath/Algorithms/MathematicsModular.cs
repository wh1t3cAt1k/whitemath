...
namespace WhiteMath.Mathematics
{
    public static partial class Mathematics<T,C> where C: ICalc<T>, new()
    {
        ...
        {
			Condition.Validate(Calculator.IsIntegerCalculator).OrException(new NonIntegerTypeException(typeof(T).Name));
...
//			
            ...
            bool evenModule = Calculator.IsEven(modulus);
			...
			// Thus, increment.
			// -
			if (Calculator.IsEven(lowerBound) && evenModule)
			{
				lowerBound++;
			}
            ...
            for (Numeric<T, C> current = lowerBound; current <= upperBound; current++)
            {
				...
				// -
                if (Mathematics<T, C>.GreatestCommonDivisor(current, modulus) != Numeric<T, C>._1)
                    goto ENDING;

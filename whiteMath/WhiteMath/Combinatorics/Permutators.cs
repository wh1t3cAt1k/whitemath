    ...
    public class LexicographicPermutator<T> : Permutator<T>
    {
        ...
        public override bool CreateNextPermutation()
        {
            ...
            for (int i = k + 1; i < maxInd; i++)
                permutation.SwapElements(i, permutation.Length - i + k);
    ...
    public class InductivePermutator<T> : Permutator<T>
    {
        ...
        private void NextPermutation(int pos)
        {
            if (trace[pos] == pos)
            {
                ...
                trace.SwapElements(pos, pos - 1);
                permutation.SwapElements(pos, pos - 1);
            }
            else // был обмен
            {
                int i;
                ...
                if (i == 0)
                {
                    trace.SwapElements(i, pos);
                    permutation.SwapElements(i, pos);
                ...
                {
                    trace.SwapElements(i, i - 1);
                    permutation.SwapElements(i, i - 1);
                }
            }

...
namespace WhiteMath.Functions
{
    ...
    public static class Interpolation<T, C> where C: ICalc<T>, new()
    {
        ...
        /// The highest continuous derivative of the spline is the second derivative.
        /// </summary>
        public static PieceFunction<T, C> CreateNaturalCubicSpline(IList<Point<T>> points, T defaultValue)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                h[i] = Calculator.Subtract(points[i+1].X, points[i].X);
                fDiv[i] = Calculator.Subtract(points[i+1].Y, points[i].Y) / h[i];
            }

    ...
    public partial class LongInt<B> where B: IBase, new()
    {
        public static partial class Helper_NTT_OLD
        {
            ...
            public static void MultiplyNTT(int BASE, IList<long> result, IList<int> one, IList<int> two)
            {
                ...
                LongInt<B>.Helper.PerformCarries(BASE, result);     // осуществим переносы
                return;
            }
            ...
            internal static BigInteger[] Recursive_NTT_Inverse(IList<BigInteger> coefficients)
            {
                ...
                BigInteger lengthMultiplicativeInverse =
                    Mathematics<BigInteger, CalcBigInteger>.MultiplicativeInverse(n, NTT_MODULUS);
            ...
            public static BigInteger[] rootsOfUnityHalfGalois(int rootDegree, bool inverted)
            {
				...
				*/
                ...
                // -
                BigInteger principalRoot = Mathematics<BigInteger, CalcBigInteger>.PowerIntegerModular(
                    NTT_MAX_ROOT_OF_UNITY_2_30,
                    Mathematics<ulong, CalcULong>.PowerInteger(2, NTT_MAX_ROOT_OF_UNITY_DEGREE_EXPONENT - rootDegreeExponent),
                    NTT_MODULUS);
                ...
                if (inverted)
                {
                    ...
                    {
                        result[i] = Mathematics<BigInteger, CalcBigInteger>.MultiplicativeInverse(result[i], NTT_MODULUS);
                    }
                }
            ...
            internal static BigInteger[] __DB_Quadratic_NTT(IList<BigInteger> coefficients, bool back = false)
            {
                ...
                for (int i = 0; i < rootsOfUnity.Length; ++i)
                {
                    ...
                    result[i] =
                        hornerResult *
                        (back ? Mathematics<BigInteger, CalcBigInteger>.MultiplicativeInverse(coefficients.Count, NTT_MODULUS) : 1) % NTT_MODULUS;
                }
